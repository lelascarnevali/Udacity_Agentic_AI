...
- 📓 [Structured Outputs — Demo](../exercises/02-structured-outputs-demo.ipynb) — demonstração completa do fluxo com `StructuredAgent`, `MeetingSummary` e `ActionItem`
- 📓 [Structured Outputs — Exercício](../exercises/02-structured-outputs-exercise.ipynb) — implemente `StructuredAgent` com `output_model`, `JsonOutputParser` e validação Pydantic
